...
const StatisticCard = ({
    ...
    return (
        <div className={classNames('bg-white p-6 rounded-2xl border border-gray-100 shadow-sm', className)}>
            <h4 className="text-teal-500 font-medium text-sm mb-2">{title}</h4>
            <div className="flex items-end gap-3">
                <span className="text-3xl font-bold text-gray-900">{value}</span>
                {growth !== undefined && (
                    <div className={classNames('flex items-center text-xs font-semibold mb-1', {
                ...
                {growthText && (
                    <span className="text-xs text-gray-400 mb-1 ml-1">{growthText}</span>
                )}
            </div>
        </div>
    )
